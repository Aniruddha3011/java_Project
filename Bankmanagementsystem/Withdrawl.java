...
public class Withdrawl extends JFrame implements ActionListener {
    ...
    Withdrawl(String pinnumber) {
        ...
        text.setBounds(310, 360, 400, 20);
        image.add(text);

        amount = new JTextField();
        amount.setFont(new Font("Roboto", Font.BOLD, 22));
        amount.setBounds(290, 400, 320, 25);
        image.add(amount);

        withdrawl = new JButton("Withdraw");
        withdrawl.setBounds(460, 495, 150, 30);
        withdrawl.addActionListener(this);
        image.add(withdrawl);

        back = new JButton("Back");
        back.setBounds(460, 530, 150, 30);
        ...
        if (ae.getSource() == withdrawl) {
           ...
           if (number.equals("")){
               JOptionPane.showMessageDialog(null, "Please enter the amount you want to Withdraw");
                ...
                JOptionPane.showMessageDialog(null, "Insuffiecient balance in your account");
           }
           else{
               try{
                    Conn conn=new Conn();
